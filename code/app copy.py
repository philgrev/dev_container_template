...
def _(color_dropdown, df, get_highlight, get_selection, go, mo, x_col_dropdown, y_col_dropdown):
    ...
    _has_sel = len(_sel) > 0
    _opacities = [1.0 if i in _sel else 0.6 for i in range(len(_x))] if _has_sel else [0.7] * len(_x)
    _sizes = [10 if i in _sel else 6 for i in range(len(_x))] if _has_sel else [6] * len(_x)
...
def _(Image, df_plot, get_current_idx, get_highlight, get_nav_pos, get_selection, image_base_path, mo, os, selected_indices, set_current_idx, set_highlight):
    ...
    if not get_selection():
        ...
        _nav = get_nav_pos() % len(selected_indices)
        _current_idx = selected_indices[_nav]
        ...
        _page_start = (_nav // _IMAGES_PER_PAGE) * _IMAGES_PER_PAGE
        _page_indices = selected_indices[_page_start : _page_start + _IMAGES_PER_PAGE]
        ...
        for _pi in _page_indices:
            ...
            _is_current = _pi == _current_idx
            _label = f"**`{_row['file_name']}`**" if _is_current else f"`{_row['file_name']}`"
            _image_cells.append(
                mo.vstack([mo.image(_img, width=160), mo.md(_label)], align="center")
            )

        image_output = mo.vstack([
            mo.md(f"**Image {_nav + 1} of {len(selected_indices)}** — Page {_page_start // _IMAGES_PER_PAGE + 1} of {_total_pages}"),
